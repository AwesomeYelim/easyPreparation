...
func VideoBgUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(300 << 20); err != nil { // 300MB
		httpx.Error(w, http.StatusBadRequest, "파일이 너무 큽니다 (최대 300MB)")
		return
	}
	...
	if ext != ".mp4" && ext != ".webm" && ext != ".mov" {
		httpx.Error(w, http.StatusBadRequest, "mp4, webm, mov 파일만 허용됩니다")
		return
	}
...
func VideoBgDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if name == "" || name == "." {
		httpx.Error(w, http.StatusBadRequest, "파일명 필요")
		return
	}
