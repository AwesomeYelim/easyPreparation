...
func (pdf *PDF) SetText(fontInfo classification.FontInfo, isB bool, textColor ...color.Color) {
	...
	// gofpdf가 경로 앞 '/'를 누락하는 버그 방지
	fontPath, _ = filepath.Abs(fontPath)
...
func (pdf *PDF) ForEdit(con types.WorshipInfo, config extract.Config) {
	...
	case "신앙고백":
		...
		pdf.SetText(fontInfo, true, hLColor)
		pdf.SetXY(pdf.Config.Padding, pdf.Config.Padding*1.5)
		pdf.MultiCell(pdf.Config.InnerRectangle.Width, pdf.Config.FontSize/1.7, con.Contents, "", "C", false)
	default:
		if con.Obj == "-" {
