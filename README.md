...
| 기능 | 설명 | 플랜 |
|------|------|------|
| **주보 생성** | Figma 디자인 기반 인쇄용(A4) + 프레젠테이션용 PDF 자동 생성 | Free |
...
LocalAuthProvider        (교회 설정 상태, SetupWizard 트리거)
  └─ LicenseProvider     (라이선스 플랜, FeatureGate)
       └─ RecoilProvider (전역 앱 상태)
            └─ WebSocketProvider
                 ├─ NavBar + SetupWizard + UpdateChecker
                 └─ { children }
```
...
| `auth.json` | Google Drive 서비스 계정 키 | 선택 |
...
| 파일 | 트리거 | 동작 |
|------|--------|------|
| `.github/workflows/release.yml` | `v*` 태그 push | 4개 플랫폼 빌드 + GitHub Release 자동 생성 |
...
| 플랫폼 | OS | 아티팩트 |
|--------|-----|----------|
| macOS ARM64 | macos-latest | `easyPreparation_darwin_arm64` |
| macOS AMD64 | macos-latest | `easyPreparation_darwin_amd64` |
| Linux AMD64 | ubuntu-latest | `easyPreparation_linux_amd64` |
| Windows AMD64 | windows-latest | `easyPreparation_windows_amd64.exe` |

빌드 단계: `checkout` → `Go 설치` → `Node 22 설치` → `Next.js 빌드` → `frontend 복사` → `Go binary 빌드 (CGO_ENABLED=0)` → `GitHub Release 생성`
